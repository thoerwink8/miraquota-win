...
- **独立于 Mirasim**：Mirasim 在线时走 `/v1/limits` 精确点数（会话令牌 PEB 自动发现）；
  关闭时优先用 fleet-dao 读到的账号额度（「账号池」页接上后生效；额度点是账号级的，谁读到都一样，
  他人占用已计入），再不行才按落盘的窗口锚点滚动推算（标 ≈，他人占用不可见故为下界）；
- **账号池（fleet-dao 的桌面窗口）**：「账号池」页列出 fleet-dao 读到的全部账号池 × 窗口——Claude
  两个组织、Mirasim、Cursor、Grok、Jev……每格标明实读还是估算、清零倒计时，上游这次没报的窗口单独注明，
  没读成的池写清原因。地址和只读令牌在那一页填，存本机 `~/.miraquota/fleet.json`，不进仓库；
...
- **多机：看别的机器跑多快**：同一账号的其他机器（含无界面的 Linux 服务器，
  `node scripts/deploy-linux.mjs --host <ssh 别名> --via-inbox` 一条命令接入）经收件口把账本与速度
  发成分片，速度卡顶部即可切到那台机器看它的模型与 tok/s。额度卡不跟着切——点数是账号级的，
  在哪台机器看都是同一个数，也不走分片（只认本机实读与 fleet-dao）。详见 [docs/MULTI-MACHINE.md](docs/MULTI-MACHINE.md)。
