...
# 2. 启动额度 provider（供数进程）。走 install.ps1 而不是直接跑 VBS：
# 它带单实例检查，重复双击不会起出第二份来抢 feed 端口。
$s2 = $ws.CreateShortcut((Join-Path $desk '额度控件 provider.lnk'))
$s2.TargetPath = 'powershell.exe'
$s2.Arguments = "-WindowStyle Hidden -ExecutionPolicy Bypass -File `"$root\scripts\install.ps1`""
