...
4. 验证：面板总览出现「多机同步」卡，状态绿标「GitHub 已接入」，每台机器一行
   「<id> · N 分钟前推送」（本机有标注）；远端仓出现 `machine/<各机器名>`
   分支且各只有 1 个提交；`~/.miraquota/sync-repo` 里只有本机的 `shard.json`。

## 故障表现

状态标记三态（payload `sync.state`）：绿「GitHub 已接入」= 最近一轮成功且不超过
2×intervalSec；红「同步失败」= 有错误（原因在 `sync.error` 首行）；灰「连接中…」=
已启用但尚无新鲜的成功记录。

| 现象 | 含义 | 处置 |
|---|---|---|
| 红标「同步失败：…」 | git push/fetch 报错（凭据、网络、远端没建） | 按所附原因修；修好后下一轮自动恢复 |
| 多机同步卡只有本机一行 | 远端只有本机分支 | 确认其他机器也配了 sync.json 并已跑过一轮 |
...
启用同步后，窗口卡的「支出」与今天卡的美元都是**全机合计**（面板多机行有注明）；
