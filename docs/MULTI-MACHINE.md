...
4. 验证：面板总览页脚 / 口径说明页出现「多机账本 ×N」；远端仓出现 `machine/<各机器名>`
...
| 现象 | 含义 | 处置 |
|---|---|---|
| 面板显示「多机同步失败」 | git push/fetch 报错（凭据、网络、远端没建） | 看 payload `sync.error` 首行；修好后下一轮自动恢复 |
| 「多机账本 · 未见其他机器」 | 远端只有本机分支 | 确认其他机器也配了 sync.json 并已跑过一轮 |
