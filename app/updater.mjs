...
// 6 小时一轮（用户 2026-09-02 拍板：频率不必高）。真正管用的是三个「用户在场」的时机：
// 启动、打开面板、点面板里的检查按钮——定时轮询只是兜底，防止应用连开几天不动。
const EVERY_MS = 6 * 60 * 60 * 1000;
// 打开面板时补查一次——那正是用户会看角标的时刻；节流防止反复开关窗口猛敲接口。
const ON_SHOW_MIN_GAP_MS = 60 * 1000;
